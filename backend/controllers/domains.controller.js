import {DeleteHostedZoneCommand, ListHostedZonesCommand, CreateHostedZoneCommand } from "@aws-sdk/client-route-53";
...
export const createHostedZoneHandler = async (req, res) => {
    try {
      const result = await createHostedZone(req, res,isDomainExist,client,CreateHostedZoneCommand);
    } catch (error) {
...
export const deleteHostedZoneHandler = async (req, res) => {
    try {
      await deleteHostedZone(req, res,isDomainExist,client,DeleteHostedZoneCommand);
    } catch (error) {
